...
internal static class HashManyAvx2
{
    ...
    // Keep the fixed eight-chunk kernel separate: carrying seven variable input offsets
    // through its inner loop regressed complete batches. Partial batches still write eight CVs.
    [SkipLocalsInit]
    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    internal static unsafe void HashManyPartial(ReadOnlySpan<byte> chunks, int numChunks,
                                       ReadOnlySpan<uint> key, ulong startCounter,
                                       uint flags, Span<uint> cvs)
    {
        const int blocksPerChunk = Blake3Constants.ChunkLen / Blake3Constants.BlockLen; // 16
        ...
        fixed (byte* chunksPtr = chunks)
        {
            ...
            for (int blockIdx = 0; blockIdx < blocksPerChunk; blockIdx++)
            {
                ...
                // Load lower 8 words (0-7) from each of 8 chunks contiguously
                var r0 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + 0 * Blake3Constants.ChunkLen);
                var r1 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset1);
                var r2 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset2);
                var r3 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset3);
                var r4 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset4);
                var r5 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset5);
                var r6 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset6);
                var r7 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset7);
                ...
                // Load upper 8 words (8-15) from each of 8 chunks contiguously
                r0 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + 0 * Blake3Constants.ChunkLen + 32);
                r1 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset1 + 32);
                r2 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset2 + 32);
                r3 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset3 + 32);
                r4 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset4 + 32);
                r5 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset5 + 32);
                r6 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset6 + 32);
                r7 = Unsafe.ReadUnaligned<Vector256<uint>>(blockBase + offset7 + 32);
